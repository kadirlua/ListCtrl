...
int CGridListCtrlEx::InsertColumnTrait(int nCol, const CString& strColumnHeading, int nFormat, int nWidth, int nSubItem, CGridColumnTrait* pTrait)
{
	...
	if (pTrait!=NULL)
	{
		...
		pTrait->GetColumnState().m_Editable = true;
...
BOOL CGridListCtrlEx::OnHeaderItemStateIconClick(UINT id, NMHDR* pNMHDR, LRESULT* pResult) {

	return FALSE;
}

void CGridListCtrlEx::SetCellBackColor(COLORREF cBackColor)
{
	m_cBackColor = cBackColor;
}

void CGridListCtrlEx::SetCellTextColor(COLORREF cTextColor)
{
	m_cTextColor = cTextColor;
}

void CGridListCtrlEx::SetTooltipText(CString strText)
{
	strTooltipText = strText;
}

void CGridListCtrlEx::SetTooltipTitle(CString strTitle)
{
	strTooltipTitleText = strTitle;
}