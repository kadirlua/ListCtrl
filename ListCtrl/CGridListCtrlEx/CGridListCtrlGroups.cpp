...
void CGridListCtrlGroups::OnContextMenuHeader(CWnd* pWnd, CPoint point, int nCol)
{
	...
	if (nCol!=-1)
	{
		...
		menu.AppendMenu(MF_STRING, 3, CString(_T("Ýle Grupla: ")) + columnTitle);
	}

	if (IsGroupViewEnabled())
	{
		menu.AppendMenu(MF_STRING, 4, _T("Gruplamayý devredýþý býrak"));
	}
...
void CGridListCtrlGroups::OnContextMenuGroup(CWnd* pWnd, CPoint point, int nGroupId)
{
	...
	if (IsGroupStateEnabled())
	{
		...
		{
			CString menuText = CString(_T("Grubu geniþlet: ")) + groupHeader;
			menu.AppendMenu(MF_STRING, 1, menuText);
		}
		else
		{
			CString menuText = CString(_T("Grubu daralt: ")) + groupHeader;
	...
	if (GetExtendedStyle() & LVS_EX_CHECKBOXES)
	{
		CString menuText = CString(_T("Grubu iþaretle: ")) + groupHeader;
		menu.AppendMenu(MF_STRING, 3, menuText);
		menuText = CString(_T("Grubu iþaretleme: ")) + groupHeader;
	...
	if (IsGroupStateEnabled())
	{
		if (menu.GetMenuItemCount() > 0)
			menu.AppendMenu(MF_SEPARATOR, 0, _T(""));
		menu.AppendMenu(MF_STRING, 5, _T("Tüm gruplarý geniþlet"));
		menu.AppendMenu(MF_STRING, 6, _T("Tüm gruplarý daralt"));
		menu.AppendMenu(MF_STRING, 7, _T("Gruplamayý devredýþý býrak"));
	}
